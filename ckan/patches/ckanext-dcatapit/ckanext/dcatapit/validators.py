...
def dcatapit_subthemes(key, flattened_data, errors, context):
    ...
    def _get_flattened_theme():
        ...
        if __extras and 'theme' in __extras:
            ...
            #log.warning('__extras nel validator %s',__extras['theme'])
            return __extras['theme']
        return None
...
#    log.warning('aggr_list %s',aggr_list)
    if not value or value == '[]':  # a little shortcut here
        theme = _get_flattened_theme()
        if theme and theme != '[]':  # other shortcut
            log.warning('Aggregate theme is missing, trying setting values from extra theme key %s',theme)
            theme_list = themes_parse_to_uris(theme)
            #log.warning('il theme_list nel validator %s',theme_list)
            _do_return(themes_to_aggr_json(theme_list))
            stat=True
#            _do_return(theme_list)
        else:
...
#    if stat==0:
 #    aggr_list = json.loads(value)
  #  else:
   #  aggr_list = json.loads(_get_flattened_theme())
  #  log.warning('aggr_list %s',aggr_list)
    try:
     if stat==False:
      aggr_list = json.loads(value)
      ...
      aggr_list = json.loads(_get_flattened_theme())
      #log.warning('aggr_list list %s',aggr_list)
    #     aggr_list = json.loads(_get_flattened_theme())
#        aggr_list = json.loads(value)
#        log.warning('aggr_list %s',json.loads(_get_flattened_theme()))
    except (TypeError, ValueError):
        # handle old '{THEME1,THEME2}' notation
        if isinstance(value, str):
            _v = value.rstrip('}').lstrip('{').split(',')
            aggr_list = [{'theme': v, 'subthemes': []} for v in _v]
            log.warning('aggr_list str %s',aggr_list)
    ...
    for aggr in aggr_list:
        ...
        #log.warning('Subthemes nel validator %s',subthemes)
        try:
